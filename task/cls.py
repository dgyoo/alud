...
class Option(object):
    ...
    def __init__(self):
        ...
        parser.add_argument('--dst-dir', default='/home/dgyoo/workspace/dataout/alud', metavar='DIR', type=str,
        ...
        parser.add_argument('--discrim-weight', default=1.0, metavar='W', type=float,
                            help='weight to the discriminator loss')
...
class ResBlock(models.ResNet):

    def __init__(self, block, layers):

        models.ResNet.__init__(self, block, layers, num_classes=1)
        del self.conv1, self.bn1, self.relu, self.maxpool, self.layer1, self.layer2, self.layer3
...
class BatchManagerTrain(Dataset):
    ...
    def __init__(self, db, opt, input_stats=None):
        ...
        self._evaluator_model = _evaluator_model
        self._evaluator_discr = _evaluator_discr
    ...
    @property
    def evaluator_discr(self):
        return self._evaluator_discr
...
def _evaluator_model(outputs, targets):
    return metric.accuracy(outputs.data, targets, topk=(1, 5))

def _evaluator_discr(outputs, targets):
    return metric.ap(outputs.data, targets)
