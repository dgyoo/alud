...
def main():
    ...
    if start_from:
        ...
        model.model.load_state_dict(checkpoint['state_dict_model'])
        model.optimizer.load_state_dict(checkpoint['optimizer_model'])
        discr.model.load_state_dict(checkpoint['state_dict_discr'])
        discr.optimizer.load_state_dict(checkpoint['optimizer_discr'])
    ...
    if opt.evaluate:
        return val.val(batch_manager_val, model, discr)
    ...
    for epoch in range(start_epoch, opt.num_epoch):
        ...
        train.train(batch_manager_train, model, discr, opt.discrim_weight, logger_train, epoch + 1)

        # Val.
        print('\nStart validation at epoch {}.'.format(epoch + 1))
        perform = val.val(batch_manager_val, model, discr, opt.discrim_weight, logger_val, epoch + 1)
        ...
        data = {
            ...
            'state_dict_model': model.model.state_dict(),
            'optimizer_model': model.optimizer.state_dict(),
            'state_dict_discr': discr.model.state_dict(),
            'optimizer_discr': discr.optimizer.state_dict()}
